...
## Pasos
0. Configurar credenciales (primera vez):
   ```bash
   cd src/aspire/AppHost/AppHost
   cp appsettings.Development.json.example appsettings.Development.json
   ```
   Edita `appsettings.Development.json` con tus credenciales si es necesario (por defecto usa valores de desarrollo).
...
- **appsettings.json**: Configuración no sensible (en repositorio)
- **appsettings.Development.json**: Credenciales (NO en repositorio, ver `.gitignore`)
- **Variables de entorno**: Prioridad más alta

Ver `docs/CONFIGURATION.md` para más detalles.
...
- Error "Host desconocido" en Postgres: La API espera automáticamente hasta 60 segundos a que Postgres esté disponible.
